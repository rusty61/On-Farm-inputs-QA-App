...
flowchart LR
  ...
  subgraph Data [Supabase (Postgres + RLS)]
    T1[owners, profiles]
    T2[farms, paddocks]
    T3[applications, application_paddocks]
    T4[blynk_stations]
  end
...
- `gps_accuracy_m` (numeric, nullable)
- `gps_updated_at` (timestamptz, nullable)
- `created_at` (timestamptz)

#### applications
- `id` (UUID, PK)
- `owner_id` (UUID, FK → owners.id)
- `mix_id` (UUID, nullable)
- `operator_user_id` (UUID, nullable)
- `started_at` (timestamptz)
- `finished_at` (timestamptz, nullable)
- `finalized` (bool)
- `notes` (text, nullable)
- `water_source` (text, nullable)
...
- `paddock_id` (UUID, FK → paddocks.id)
- `gps_latitude` (numeric, nullable)
- `gps_longitude` (numeric, nullable)
- `gps_accuracy_m` (numeric, nullable)
- `gps_captured_at` (timestamptz, nullable)
